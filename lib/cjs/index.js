...
var SendNdebitRequest = async (pool, privateKey, relays, pubKey, data) => {
  ...
  const res = await pool.get(relays, newNip68Filter(publicKey, signed.id), { maxWait: 30 * 1e3 });
  if (!res) {
    throw new Error("failed to get nip68 response in time");
  }
  decrypt2(res.content, getConversationKey(privateKey, pubKey));
...
var SendNofferRequest = async (pool, privateKey, relays, pubKey, data) => {
  ...
  const signed = finalizeEvent(event, privateKey);
  pool.publish(relays, signed);
  const res = await pool.get(relays, newNip69Filter(publicKey, signed.id), { maxWait: 30 * 1e3 });
  if (!res) {
