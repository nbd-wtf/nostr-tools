    ...
    case "noffer": {
      ...
      return {
        ...
        data: {
          ...
          priceType: tlv[3][0][0],
          price: tlv[4] ? parseInt((0, import_utils5.bytesToHex)(tlv[4][0]), 16) : void 0
        }
      };
    }
    case "ndebit": {
...
var nofferEncode = (offer) => {
  const o = {
    0: [(0, import_utils5.hexToBytes)(offer.pubkey)],
...
var ndebitEncode = (debit) => {
  const o = {
    0: [(0, import_utils5.hexToBytes)(debit.pubkey)],
...
var SendNdebitRequest = async (pool, privateKey, relays, pubKey, data) => {
  const privateBytes = (0, import_utils13.hexToBytes)(privateKey);
  const publicKey = getPublicKey(privateBytes);
  const content = encrypt2(JSON.stringify(data), getConversationKey(privateBytes, pubKey));
  ...
  decrypt2(res.content, getConversationKey(privateBytes, pubKey));
...
var SendNofferRequest = async (pool, privateKey, relays, pubKey, data) => {
  const privateBytes = (0, import_utils14.hexToBytes)(privateKey);
  const publicKey = getPublicKey(privateBytes);
  const content = encrypt2(JSON.stringify(data), getConversationKey(privateBytes, pubKey));
  const event = newNip69Event(content, publicKey, pubKey);
  const signed = finalizeEvent(event, privateBytes);
  pool.publish(relays, signed);
  const res = await pool.get(relays, newNip69Filter(pubKey, signed.id), { maxWait: 30 * 1e3 });
  if (!res) {
