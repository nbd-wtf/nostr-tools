...
var NostrTools = (() => {
  ...
  var SendNdebitRequest = async (pool, privateKey, relays, pubKey, data) => {
    const privateBytes = hexToBytes2(privateKey);
    const publicKey = getPublicKey(privateBytes);
    const content = encrypt3(JSON.stringify(data), getConversationKey(privateBytes, pubKey));
    ...
    decrypt3(res.content, getConversationKey(privateBytes, pubKey));
