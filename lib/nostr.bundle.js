...
var NostrTools = (() => {
    ...
    return new Promise((res, rej) => {
      ...
      closer = pool.subscribeMany(relays, [newNip68Filter(publicKey, signed.id)], {
        onevent: async (e) => {
          if (timer)
            clearTimeout(timer);
          const content2 = decrypt3(e.content, getConversationKey(privateKey, pubKey));
  ...
  var SendNofferRequest = async (pool, privateKey, relays, toPubKey, data, timeoutSeconds = 30) => {
    const publicKey = getPublicKey(privateKey);
    const content = encrypt3(JSON.stringify(data), getConversationKey(privateKey, toPubKey));
    ...
    return new Promise((res, rej) => {
      ...
      closer = pool.subscribeMany(relays, [newNip69Filter(publicKey, signed.id)], {
        onevent: async (e) => {
          ...
          const content2 = decrypt3(e.content, getConversationKey(privateKey, toPubKey));
          res(JSON.parse(content2));
        }
      });
    });
