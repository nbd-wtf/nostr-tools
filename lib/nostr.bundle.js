...
zoo`.split("\n");
  ...
  var HMAC3 = class extends Hash {
    constructor(hash3, _key) {
      ...
      this.outputLen = this.iHash.outputLen;
      const pad = new Uint8Array(blockLen);
      pad.set(key.length > this.iHash.blockLen ? hash3.create().update(key).digest() : key);
      for (let i = 0; i < pad.length; i++)
        pad[i] ^= 54;
      this.iHash.update(pad);
      this.oHash = hash3.create();
  ...
  function extract(hash3, ikm, salt) {
    ...
    return hmac3(hash3, toBytes(salt), toBytes(ikm));
  }
  ...
  var hkdf = (hash3, ikm, salt, info, length) => expand(hash3, extract(hash3, ikm, salt), info, length);
  ...
  async function privateKeyFromX(username, caip10, sig, password) {
    ...
    let inputKey = await sha256(utils.hexToBytes(sig.toLowerCase().startsWith("0x") ? sig.slice(2) : sig));
    ...
    return utils.bytesToHex(utils.hashToPrivateKey(hashKey));
  }
